// Sistema de backup e recuperaÁ„o de dados
...
class BackupService {
  ...
  async createFullBackup(options: BackupOptions = {}): Promise<BackupData> {
    try {
      ...
      for (const tableName of finalTables) {
        try {
          ...
          if (error) {
            ...
            console.log(`‚úì Backup da tabela ${tableName}: ${(data || []).length} registros`);
          }
        } catch (err) {
      ...
      console.log(`‚úÖ Backup concluÌdo: ${totalRecords} registros, ${backupData.metadata.backup_size}`);
  ...
  async exportBackupToFile(backupData: BackupData, filename?: string): Promise<void> {
    try {
      ...
      console.log(`‚úÖ Backup exportado: ${finalFilename}`);
    } catch (error) {
  ...
  async importBackupFromFile(file: File): Promise<BackupData> {
    try {
      ...
      console.log(`‚úÖ Backup importado: vers„o ${backupData.version}, ${backupData.metadata.total_records} registros`);
  ...
  async restoreFromBackup(backupData: BackupData, options: { 
    ...
    try {
      console.log('Iniciando restauraÁ„o do backup...');
      ...
      for (const tableName of tablesToProcess) {
        ...
        if (!tableData || tableData.length === 0) {
          console.log(`‚è≠Ô∏è Pulando tabela ${tableName}: sem dados`);
          continue;
        }
        ...
        try {
          ...
          if (clearExisting) {
            console.log(`üóëÔ∏è Limpando dados existentes da tabela ${tableName}`);
            const { error: deleteError } = await supabase
              .from(tableName)
              .delete()
              .neq('id', '00000000-0000-0000-0000-000000000000'); // Deletar todos exceto um ID impossÌvel
          ...
          for (let i = 0; i < tableData.length; i += batchSize) {
            ...
            if (insertError) {
              ...
              console.log(`‚úì Lote ${Math.floor(i/batchSize) + 1}/${Math.ceil(tableData.length/batchSize)} da tabela ${tableName}`);
            }
          }
          
          console.log(`‚úÖ Tabela ${tableName} restaurada com sucesso`);
        } catch (err) {
          console.error(`Erro ao restaurar tabela ${tableName}:`, err);
        }
      }
      
      console.log('üéâ RestauraÁ„o do backup concluÌda!');
    } catch (error) {
  ...
  // Criar backup autom·tico (pode ser chamado periodicamente)
  async createAutomaticBackup(): Promise<void> {
    try {
      console.log('Executando backup autom·tico...');
      ...
      console.log('‚úÖ Backup autom·tico concluÌdo');
    } catch (error) {
  ...
  async checkDataIntegrity(): Promise<{
    ...
    try {
      ...
      // VerificaÁıes especÌficas de integridade
      ...
      // 2. Verificar se existem itens de venda Ûrf„os
      ...
      if (orphanSaleItems && orphanSaleItems.length > 0) {
        issues.push(`${orphanSaleItems.length} itens de venda Ûrf„os encontrados`);
      }
      ...
      console.log(`VerificaÁ„o de integridade concluÌda: ${issues.length} problemas encontrados`);
      ...
      console.error('Erro na verificaÁ„o de integridade:', error);
      return {
        isValid: false,
        issues: [`Erro na verificaÁ„o: ${error}`],
        summary
      };
    }
  }
...
// Inst√¢ncia singleton do serviÁo de backup
export const backupService = new BackupService();

// FunÁıes de conveniÍncia para uso direto
