...
class BackupService {
  ...
  async createFullBackup(options: BackupOptions = {}): Promise<BackupData> {
    try {
      ...
      console.log(`✅ Backup concluído: ${totalRecords} registros, ${backupData.metadata.backup_size}`);
  ...
  async importBackupFromFile(file: File): Promise<BackupData> {
    try {
      ...
      console.log(`✅ Backup importado: versão ${backupData.version}, ${backupData.metadata.total_records} registros`);
